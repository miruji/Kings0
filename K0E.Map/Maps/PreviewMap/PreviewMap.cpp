...
bool Test3(vector< vector<size_t> >& Map, vector< vector< size_t > > TeamMap, size_t i, size_t j, string Data)
{
	// Read data 
	for (size_t ii = 0; ii < Data.length(); ii++) 
	{
		...
		if (i >= 1 && j >= 1 && i <= TeamMap.size()-1 && j <= TeamMap.size()-1)
		{
			// Up 
			if (ii == 0)
			if (Data[0] == '0') {
...
PreviewMap::PreviewMap()
{
	...
	DomainTexture.loadFromFile("Data/Textures/Map/PreviewMap/PreviewMap.png");
	DomainBorderTexture.loadFromFile("Data/Textures/Map/PreviewMap/PreviewMapBorder.png");
	Sprite.setTexture(DomainTexture);
}

// Generate 
void PreviewMap::Generate(vector< vector<MapBlock> >& BlocksVector, vector< vector<size_t> > TeamMap)
{
	// Generate map 
...
// Update border map 
void PreviewMap::UpdateBorderMap()
{
	// ... 
	BorderMap = TeamMap;
	...
	for (size_t j = 0; j < BorderMap[i].size(); j++)
	{
		...
		{
			// Panel 
			if ( Test3(BorderMap, TeamMap, i, j, "--00NNNN") )
				BorderMap[i][j] = 14;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "00--NNNN") )
				BorderMap[i][j] = 15;
			else
			// Arc 
			if ( Test3(BorderMap, TeamMap, i, j, "0---NNNN") )
				BorderMap[i][j] = 13;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "--0-NNNN") )
				BorderMap[i][j] = 12;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "---0NNNN") )
				BorderMap[i][j] = 11;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "-0--NNNN") )
				BorderMap[i][j] = 10;
			...
			// Basic + 0 
			if ( Test3(BorderMap, TeamMap, i, j, "0000NNNN"))
				BorderMap[i][j] = 16;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "-N00NNNN") )
				BorderMap[i][j] = 2;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "N-00NNNN") )
				BorderMap[i][j] = 4;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "00-NNNNN") )
				BorderMap[i][j] = 3;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "00N-NNNN") )
				BorderMap[i][j] = 5;
			else
			// Angles 
			if ( Test3(BorderMap, TeamMap, i, j, "-0-0NNNN") )
				BorderMap[i][j] = 6;
			else
			if ( Test3(BorderMap, TeamMap, i, j, "-00-NNNN") )
				BorderMap[i][j] = 7;
			...
			if ( Test3(BorderMap, TeamMap, i, j, "0-0-NNNN") )
				BorderMap[i][j] = 9;
			// Else 
			else
			//	BorderMap[i][j] = 17; // 1 
...
// Get team map (link) 
vector< vector< size_t > >& PreviewMap::GetTeamMap()
{
	return TeamMap;
}

// Draw 
void PreviewMap::Draw(sf::RenderWindow& Render, float CameraX, float CameraY, float CameraW, float CameraH, std::vector< std::vector<MapBlock> >& BlocksVector, bool DrawingWarFog)
{
	//
	for (size_t i = 0; i < BlocksVector.size(); i++) 
	for (size_t j = 0; j < BlocksVector[i].size(); j++) 
	{
			 ...
			 j <= size_t((CameraX+CameraW)/32) && i <= size_t((CameraY+CameraH)/32))
		{
			...
			if (BorderMap[i][j] != 16)
				Sprite.setTextureRect(sf::IntRect(BorderMap[i][j]*32, 0, 32, 32));
			else
				Sprite.setTextureRect(sf::IntRect(0, 0, 32, 32));
			Render.draw(Sprite);
		}
