...
// Control X and Y: Q E Z C 
bool Window::Control_QEZC(char Direction, bool Attack)
{
	//
	bool Result = true;

	if (Event.key.code == sf::Keyboard::Q || Event.key.code == sf::Keyboard::E ||
		Event.key.code == sf::Keyboard::Z || Event.key.code == sf::Keyboard::C || Direction != ' ')
	{
		// Message 
		if (UI1->Message != 0)
		{
		...
		// Control
		// Q
		if ((Event.key.code == sf::Keyboard::Q && Direction == ' ') || Direction == 'Q')
		{
			NextBlock = LandMap1->BlocksVector[size_t(Game1->Player1->Y/32)-1][size_t(Game1->Player1->X/32)-1];
			...
			if (SearchNextBlockEnemy(NextBlock, Game1->Player1->Team) || Attack)
			{
				...
				if ((Technologys[1].Opened || NextBlock.Mountain == false) && ((Technologys[3].Opened && NextBlock.River == true) || (Technologys[2].Opened && Technologys[3].Opened) || (NextBlock.River == false && NextBlock.Land == true)) && (Technologys[0].Opened || NextBlock.Forest == false))
				{
					...
					LandMap1->BlocksVector[size_t(Game1->Player1->Y/32)-1][size_t(Game1->Player1->X/32)-1].Enemy = Game1->Player1->Team;
				}
				else
		...
		// E
		if ( (Event.key.code == sf::Keyboard::E && Direction == ' ') || Direction == 'E' )
		{
			NextBlock = LandMap1->BlocksVector[size_t(Game1->Player1->Y/32)-1][size_t(Game1->Player1->X/32)+1];
			if (DomainMap1->GetTeamMap()[NextBlock.Y/32][NextBlock.X/32] == 1)
				SteamAPI1->SetAchievement("Boundaries");
		...
		if ( (Event.key.code == sf::Keyboard::Z && Direction == ' ') || Direction == 'Z' )
		{
			NextBlock = LandMap1->BlocksVector[size_t(Game1->Player1->Y/32)+1][size_t(Game1->Player1->X/32-1)];
			if (DomainMap1->GetTeamMap()[NextBlock.Y/32][NextBlock.X/32] == 1)
				SteamAPI1->SetAchievement("Boundaries");
		...
		if ( (Event.key.code == sf::Keyboard::C && Direction == ' ') || Direction == 'C' )
		{
			NextBlock = LandMap1->BlocksVector[size_t(Game1->Player1->Y/32)+1][size_t(Game1->Player1->X/32)+1];
			if (DomainMap1->GetTeamMap()[NextBlock.Y/32][NextBlock.X/32] == 1)
				SteamAPI1->SetAchievement("Boundaries");
